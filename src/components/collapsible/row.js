...
class Row extends React.Component {
  ...
  updateIsDesktop() {
    ...
    if (headLen > 1) {
      this.setState({
        isDesktop:
          this.state.headList[0].getBoundingClientRect().top ===
          this.state.headList[headLen - 1].getBoundingClientRect().top
      });
    }
  }
      ...
      <>
        <BootstrapRow className={classnames(this.props.className)}>
          {React.Children.map(this.props.children || [], (child, colId) =>
            child.type.displayName !== "Collapsible.Col"
              ? child
        ...
        {React.Children.map(
          ...
          (child, colId) =>
            child.props.body && (
              <Body
                expanded={
                  `${this.props.id}-${colId}` === this.props.expandedId &&
